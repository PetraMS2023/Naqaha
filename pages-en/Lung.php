<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <!-- Logo In Browser -->
    <link rel="sortcut icon" type="image/jpg" href="../media/logos/logo browser.png" />
    <!-- Font Awesome Library -->
    <link rel="stylesheet" href="../CSS/all.min.css" />
    <!-- Render All Element Normally -->
    <link rel="stylesheet" href="../CSS/normalize.css" />
    <!-- Main Template CSS -->
    <link rel="stylesheet" href="../CSS/design.css" />
    <!-- Slider File CSS -->
    <link rel="stylesheet" href="../js/swiper-bundle.min.css" />
    <!-- SCSS File -->
    <link rel="stylesheet" href="../scss/style.css" />
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Urbanist:ital,wght@0,100..900;1,100..900&display=swap" rel="stylesheet">
    <title>Internal Medicine NQAHA</title>
</head>

<body>
<?php include  "../parts/en/Header.php" ?>

    <!-- Start Landing -->
    <div class="landing">
        <div class="container">
            <div class="title btmAni25002">the Lung</div>
            <div class="sub-title btmAni20002">
                <span>Home / </span>
                <span class="super">Internal Medicine</span>
            </div>
        </div>
    </div>
    <!-- End Landing -->
    <!-- Start Go UP -->
    <div class="goTop js-scroll fade-btm">
        <span>
            <i class="fa-solid fa-angle-up"></i>
        </span>
    </div>
    <!-- End Go Up -->
    <!-- Start Heart -->
    <div class="disease">
        <div class="container">
            <div class="title">Lung disease</div>
            <div class="faf">
                <div class="faqs">
                    <div class="faq js-scroll fade-btm">
                        <div class="top">
                            <h3>Coronary artery disease (CAD):</h3>
                            <img class="fa-regular fa-angle-down"  src="../media/icons/arrow-down.svg" alt="">
                            <i class="fa-solid fa-circle-xmark"></i>
                        </div>
                        <p>
                            It occurs when the coronary arteries that supply 
                            blood to the heart are narrowed or blocked due to 
                            the buildup of fatty plaques (atherosclerosis).
                        </p>
                    </div>
                </div>
                <div class="faqs">
                    <div class="faq js-scroll fade-btm">
                        <div class="top">
                            <h3>Coronary artery disease (CAD):</h3>
                            <img class="fa-regular fa-angle-down"  src="../media/icons/arrow-down.svg" alt="">
                            <i class="fa-solid fa-circle-xmark"></i>
                        </div>
                        <p>
                            It occurs when the coronary arteries that supply 
                            blood to the heart are narrowed or blocked due to 
                            the buildup of fatty plaques (atherosclerosis).
                        </p>
                    </div>
                </div>
                <div class="faqs">
                    <div class="faq js-scroll fade-btm">
                        <div class="top">
                            <h3>Coronary artery disease (CAD):</h3>
                            <img class="fa-regular fa-angle-down"  src="../media/icons/arrow-down.svg" alt="">
                            <i class="fa-solid fa-circle-xmark"></i>
                        </div>
                        <p>
                            It occurs when the coronary arteries that supply 
                            blood to the heart are narrowed or blocked due to 
                            the buildup of fatty plaques (atherosclerosis).
                        </p>
                    </div>
                </div>
                <div class="faqs">
                    <div class="faq js-scroll fade-btm">
                        <div class="top">
                            <h3>Coronary artery disease (CAD):</h3>
                            <img class="fa-regular fa-angle-down"  src="../media/icons/arrow-down.svg" alt="">
                            <i class="fa-solid fa-circle-xmark"></i>
                        </div>
                        <p>
                            It occurs when the coronary arteries that supply 
                            blood to the heart are narrowed or blocked due to 
                            the buildup of fatty plaques (atherosclerosis).
                        </p>
                    </div>
                </div>
                <div class="faqs">
                    <div class="faq js-scroll fade-btm">
                        <div class="top">
                            <h3>Coronary artery disease (CAD):</h3>
                            <img class="fa-regular fa-angle-down"  src="../media/icons/arrow-down.svg" alt="">
                            <i class="fa-solid fa-circle-xmark"></i>
                        </div>
                        <p>
                            It occurs when the coronary arteries that supply 
                            blood to the heart are narrowed or blocked due to 
                            the buildup of fatty plaques (atherosclerosis).
                        </p>
                    </div>
                </div>
                <div class="faqs">
                    <div class="faq js-scroll fade-btm">
                        <div class="top">
                            <h3>Coronary artery disease (CAD):</h3>
                            <img class="fa-regular fa-angle-down"  src="../media/icons/arrow-down.svg" alt="">
                            <i class="fa-solid fa-circle-xmark"></i>
                        </div>
                        <p>
                            It occurs when the coronary arteries that supply 
                            blood to the heart are narrowed or blocked due to 
                            the buildup of fatty plaques (atherosclerosis).
                        </p>
                    </div>
                </div>
            </div>
            <div class="btnDoc">
                <a href="../pages-en/doctor.php" class="btn js-scroll fade-btm">
                    show doctors
                    <div class="image">
                        <img src="../media/icons/right-arrow-btn.svg" alt="">
                    </div>
                </a>
            </div>
        </div>
    </div>
    <!-- End Heart -->
    <!-- S -->
    <!-- Start Footer -->
    <div class="footer">
        <img class="back" src="../media/footer-back.svg" alt="">
        <div class="container">
            <div class="top">
                <div class="left">
                    <div class="logo">
                        <img src="../media/logos/logo-foooter.svg" alt="">
                    </div>
                    <p>
                        Lorem ipsum dolor sit amet consectetur adipisicing elit.
                    </p>
                </div>
                <div class="right">
                    <div class="helpful-link">
                        <div class="title">helpful link</div>
                        <ul>
                            <li>
                                <img src="../media/icons/arrow-footer.svg" alt="">
                                <a href="../index.php">Home</a>
                            </li>
                            <li>
                                <img src="../media/icons/arrow-footer.svg" alt="">
                                <a href="../pages-en/about.php">about us</a>
                            </li>
                            <li>
                                <img src="../media/icons/arrow-footer.svg" alt="">
                                <a href="../pages-en/doctor.php">doctors</a>
                            </li>
                            <li>
                                <img src="../media/icons/arrow-footer.svg" alt="">
                                <a href="../pages-en/departments.php">Medical departments</a>
                            </li>
                            <li>
                                <img src="../media/icons/arrow-footer.svg" alt="">
                                <a href="../pages-en/contact.php">contact us</a>
                            </li>
                        </ul>
                    </div>
                    <div class="contact-us">
                        <div class="title">Contact US</div>
                        <ul>
                            <li>
                                <img src="../media/icons/location-footer.svg" alt="">
                                <a href="https://maps.app.goo.gl/3bH47JZtEmXNneqU6?g_st=iw">North Marka, Street Fuheis,
                                    Amman</a>
                            </li>
                            <li>
                                <img src="../media/icons/phone-footer.svg" alt="">
                                <a href="tel:+962 7 8109 1090">+962 7 8109 1090</a>
                            </li>
<li>
                                <img src="../media/icons/phone-footer.svg" alt="">
                                <a href="tel:0048665092088">0048665092088</a>
                            </li>
                            <li>
                                <img src="../media/icons/email-footer.svg" alt="">
                                <a href="mailto:fghz2014@gmail.com">fghz2014@gmail.com</a>
                            </li>
                        </ul>
                        <div class="so">
                            <a href="#">
                                <img src="../media/icons/facebook.svg" alt="">
                            </a>
                            <a href="#">
                                <img src="../media/icons/instagrem.svg" alt="">
                            </a>
                            <a
                                href="#">
                                <img src="../media/icons/twiter-footer.svg" alt="">
                            </a>
                            <a
                                href="#">
                                <img src="../media/icons/whatsapp.svg" alt="">
                            </a>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="bottom">
            <p>
                Copyright Design © Nqaha. All right reserved
                Developed by <a href="https://petra-ms.com/"> Petra MS</a>
            </p>
        </div>
    </div>
    <!-- End Footer -->
    <!-- Start JS Files -->
    <script src="../js/main.js"></script>
    <script src="../js/swiper-bundle.min.js"></script>
    <script src="../js/swiper.js"></script>

    <script src="../js/scroll.js"></script>
    <script src="../js/sc.js"></script>
    <script src="../js/filters.js"></script>
    <!-- End JS Files -->
</body>

</html>